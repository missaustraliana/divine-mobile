...
- If you add a `*_service.dart` under `mobile/lib/services/`, add its same-named `*_test.dart` and ratchet the untested-services floor: `UPDATE_BASELINE=1 bash mobile/scripts/check_untested_services_floor.sh`, then commit the updated `mobile/scripts/baseline/untested_services.txt`. The floor may only shrink; shipping a new untested service fails CI (`check_untested_services_floor.sh`) and the pre-push hook.
